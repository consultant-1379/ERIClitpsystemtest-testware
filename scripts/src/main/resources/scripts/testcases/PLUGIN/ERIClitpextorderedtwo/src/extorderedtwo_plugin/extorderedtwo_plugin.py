...
class ExtOrderedTwoPlugin(Plugin):
    """
    LITP extorderedtwo plugin
    """
...
#pylint: disable=W0613
    def create_file(self, callback_api, *args, **kwargs):
        ...
        """
        log.trace.info("EXORD2:create_file callback called")
        filename = args[0]
        log.trace.info("EXORD2:This should create file %s" % filename)
    ...
    def delete_file(self, callback_api, *args, **kwargs):
        ...
        """
        log.trace.info("EXORD2:delete_file callback called")
        filename = args[0]
        log.trace.info("EXORD2:This should delete file %s" % filename)
        try:
            os.remove(filename)
        except OSError:
    ...
    def create_configuration(self, plugin_api_context):
        ...
        *Example CLI for this plugin:*
        ...
        .. code-block:: bash
          ...
          # TODO Please provide an example CLI snippet for plugin extorderedtwo
          # here
        """
        tasks = []
        ...
        for node in nodes:
            ...
            for pk in pklists:
                if "ORDTEST" not in pk.name:
                    continue
                filename1 = "/etc/example_ord2_conf1_%s_%s" % \
                     (node.hostname, pk.name)
                filename2 = "/etc/example_ord2_conf2_%s_%s" % \
                     (node.hostname, pk.name)
                filename3 = "/etc/example_ord2_call1_%s_%s" % \
                     (node.hostname, pk.name)
                filename4 = "/etc/example_ord2_call2_%s_%s" % \
                     (node.hostname, pk.name)
                if pk.is_initial() or pk.is_updated():
                    log.trace.debug("EXORD2:Create task %s" % filename4)
                    log.trace.debug("EXORD2:Create task %s" % filename3)
                    log.trace.debug("EXORD2:Create task %s" % filename2)
                    log.trace.debug("EXORD2:Create task %s" % filename1)
                ...
                if pk.is_updated():
                    ...
                    if oldname != pk.name:
                        oldfile1 = "/etc/example_ord2_conf1_%s_%s" % \
                           (node.hostname, oldname)
                        oldfile2 = "/etc/example_ord2_conf2_%s_%s" % \
                           (node.hostname, oldname)
                        oldfile3 = "/etc/example_ord2_call1_%s_%s" % \
                           (node.hostname, oldname)
                        oldfile4 = "/etc/example_ord2_call2_%s_%s" % \
                           (node.hostname, oldname)
                        log.trace.debug("EXORD2:remove old file %s" % oldfile1)
                        tasks.append(ConfigTask(node,
                                       pk,
                                       "Remove file %s" % oldfile1,
                                       "file", oldfile1,
                                       ensure="absent"))
                        log.trace.debug("EXORD2:remove old file %s" % oldfile2)
                        tasks.append(ConfigTask(node,
                                       pk,
                                       "Remove file %s" % oldfile2,
                                       "file", oldfile2,
                                       ensure="absent"))
                        log.trace.debug("EXORD2:remove old file %s" % oldfile3)
                        tasks.append(CallbackTask(pk,
                                       "Remove file %s" % oldfile3,
                                       self.delete_file,
                                       oldfile3))
                        log.trace.debug("EXORD2:remove old file %s" % oldfile4)
                        tasks.append(CallbackTask(pk,
