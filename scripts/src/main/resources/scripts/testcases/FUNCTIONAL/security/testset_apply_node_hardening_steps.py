...
class ApplyNodeHardeningSteps(GenericTest):
    ...
    def tearDown(self):
        ...
        fnames = [("/tmp/a_testset_apply_node_hardening_steps.ApplyNodeHarden"
                   "ingSteps.test_01_p_update_login_and_shell_settings.xml"),
                  ("/tmp/b_testset_apply_node_hardening_steps.ApplyNodeHarde"
                   "ningSteps.test_01_p_update_login_and_shell_settings.xml")]
        for fname in fnames:
            cmd = "rm -f " + fname
    ...
    def encrypt_grub_password(self, node):
        ...
        3. Enable password login for single user mode in RedHat 6.
        """
        def change_string(node, old_string, new_string, file_name):
            """
            Used to edit or add a line to a file.
            """
            cmd = '/bin/sed -i "s|{0}|{1}|" {2}'.format(old_string,
                                                        new_string,
                                                        file_name)
            self.log("info", "CMD is: " + cmd)
            self.run_command(node, cmd, su_root=True)

        # 1. Encrypt the password.
        self.log("info", "Adding a grub password hash.")
        cmd = "/sbin/grub-md5-crypt"
        expects_cmds = list()
        expects_cmds.append(self.get_expects_dict("Password:", "@dm1nS3rv3r"))
        expects_cmds.append(self.get_expects_dict("Retype password:",
                                                  "@dm1nS3rv3r"))
        std_out, _, _ = self.run_expects_command(node, cmd,
                                                 expects_cmds)

        hashed = std_out[0].replace("$", "\\$")
        self.log("info", "Hashed is: " + hashed)

        # 2. Edit /boot/grub/grub.conf
        conf_string = 'timeout=10\\npassword --md5 {0}'.format(hashed)

        change_string(node, '^timeout=10', conf_string,
                      '/boot/grub/grub.conf')
        ...
        expects_cmds = list()
        expects_cmds.append(self.get_expects_dict("Password:", "@dm1nS3rv3r"))
        self.run_expects_command(node, cmd, expects_cmds)

        self.log("info", "Enabling password login for single user mode.")
        # Ensure it is not set already.
        cmd = 'cat /etc/sysconfig/init'
        std_out, _, _ = self.run_command(node, cmd, su_root=True)
        if 'SINGLE=/sbin/sushell' in std_out:
            change_string(node, 'SINGLE=/sbin/sushell',
                          'SINGLE=/sbin/sulogin',
                          '/etc/sysconfig/init')

        self.assertTrue(self.is_text_in_list('SINGLE=/sbin/sulogin', std_out),
    ...
    def pwd_login_for_single_user_mode(self):
        """
        Description:
        Enables password login for single user mode
        """
        old_string = "SINGLE=/sbin/sushell"
        new_string = "SINGLE=/sbin/sulogin"
        f_name = "/etc/sysconfig/init"
        for node in self.all_nodes:
            self.log("info",
                     "Enabling password login for single user mode on :{0}"
                     .format(node))
            self.change_str_in_file(node, old_string, new_string, f_name)
    ...
    @attr("hardening", "shell", "non-revert")
    def test_01_p_update_login_and_shell_settings(self):
        """
            Description:
                ...
                -edit umask settings
                -enable passowrd login for single user mode
        ...
        self.pwd_login_for_single_user_mode()
    ...
    @attr("hardening", "network", "non-revert")
