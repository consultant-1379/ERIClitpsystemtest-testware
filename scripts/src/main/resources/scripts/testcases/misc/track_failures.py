...
class TrackFailures(object):
	...
	if self.p1_num:
		...
		self.__output_failure_history(self.p1_num, "2_1_ST_Functional", "ST_Functional TESTS", "20")
	#if self.p2_num:
    ...
    def __output_report_link(self, job_name, job_num, cdb=False):
	    ...
	    report_url = "http://10.44.86.30/jenkins/view/2_1_ST/job/{0}/{1}/artifact/tmpSTMB/src/test_runner/test_result/test_report.html".format(job_name, job_num)
	...
	for index in range(0, history_length_int):
		...
		fail_url = "http://10.44.86.30/jenkins/view/2_1_ST/job/{0}/{1}/artifact/tmpSTMB/src/test_runner/test_result/test_failure_file.txt".format(job_name, str(job_num_int))
                try:
                    page_contents = urllib2.urlopen(fail_url).read()
                except Exception, e:
